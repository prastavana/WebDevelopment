...
const Cart: React.FC = () => {
    ...
    const {data,refetch}=useQuery({
        ...
        queryFn(){
            return axios.get(`http://localhost:8020/cart/getByUserId/${userId}`, {
    ...
    const deleteApi = useMutation({
        ...
        mutationFn(id: number) {
            return axios.delete("http://localhost:8020/cart/deleteById/"+id, {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
            })
        },
        onSuccess() {
    ...
    const checkoutApi=useMutation({
        ...
        mutationFn(payload){
            return axios.post("http://localhost:8020/order/saveAll",payload,{
                headers:{
